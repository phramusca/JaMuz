...
//            fileInfoListTemp = new ArrayList(new HashSet(fileInfoListTemp)); //no order
            ...
            if (!Jamuz.getDb().file().getFiles(fileInfoListTemp, destExt, this.getSqlWhere())) {
                return false;
            }
        }
    ...
    public static class Filter {
        ...
        public String getQuery() {
            ...
            switch (this.field) {
                case TAG:
                    ...
                    sql.append("""
                               (SELECT F.idFile FROM file F JOIN tagFile TF ON TF.idFile=F.idFile 
        ...
        public String toString() {
            ...
            if (this.field.equals(Field.PLAYLIST)) {
                valueToReturn = Jamuz.getPlaylist(Integer.parseInt(this.value)).name;
            }
            if (this.field.equals(Field.CHECKEDFLAG)) {
                valueToReturn = CheckedFlag.values()[Integer.parseInt(this.value)].toString();
            }
            if (this.field.equals(Field.COPYRIGHT)) {
                valueToReturn = CopyRight.values()[Integer.parseInt(this.value)].toString();
            }
            switch (field) {
                case ALBUMRATING:
                case PERCENTRATED:
