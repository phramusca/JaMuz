...
public class DaoClient {
    ...
    private boolean get(LinkedHashMap<Integer, ClientInfo> clients, String login) {
        try (PreparedStatement stSelectClients = prepareClientStatement(login)) {
            try (ResultSet rs = stSelectClients.executeQuery()) {
                while (rs.next()) {
    ...
    /**
     * Prepare statement for retrieving clients based on login
     *
     * @param login
     * @return PreparedStatement
     * @throws SQLException
     */
    private PreparedStatement prepareClientStatement(String login) throws SQLException {
        String sql = """
            ...
            JOIN statSource S ON S.idStatSource=C.idStatSource"""
                + (login.isBlank() ? "" : " WHERE login=? ");

        PreparedStatement stSelectClients = dbConn.connection.prepareStatement(sql); // NOI18N
        if (!login.isBlank()) {
            stSelectClients.setString(1, login);
        }
        return stSelectClients;
    }
