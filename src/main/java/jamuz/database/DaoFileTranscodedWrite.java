 ...
 * @author raph
 */
public class DaoFileTranscodedWrite {
     ...
     * @param dbConn
     */
    public DaoFileTranscodedWrite(DbConn dbConn) {
        this.dbConn = dbConn;
    }
            ...
            try {
                if (!files.isEmpty()) {
                    ...
                    String sql = """
                        ...
                        (idFile, ext, bitRate, format, length, size, trackGain, albumGain, modifDate)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ON CONFLICT(idFile, ext) DO UPDATE SET
                        bitRate=?, format=?, length=?, size=?, trackGain=?, albumGain=?, modifDate=?
                        """;
                    try (PreparedStatement preparedStatement = dbConn.connection.prepareStatement(sql)) {
