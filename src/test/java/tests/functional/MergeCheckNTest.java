...
public class MergeCheckNTest extends TestCase {
	...
	public void test() throws Exception {
		...
		**********************************************************
		...
		/**
		 * *********************************************************
		 * Scan library quick to insert files in JaMuz Check Db and FS
		**********************************************************
		 */
		...
		/**
		 * *********************************************************
		 * Update some rating 0 -> 4 in JaMuz & check -> to have enough (but not
		 * too many) files in test playlist to export to device MyTunes
		 * (Android)
		**********************************************************
		 */
		for (String mbId : mbIds) {
		...
		**********************************************************
		...
		/**
		 * *********************************************************
		 * 1st Merge & check Then change some stats in stat sources & JaMuz
		 * (+check)
		**********************************************************
		 */
...
//            PanelCheck.addToActionQueue(folder);//FIXME TEST !!!
		...
		/**
		 * *********************************************************
		 * Check library again and check changes have been applied Then apply
		 * changes (OK selected)
		**********************************************************
		 */
		TestProcessHelper.checkLibrary();
		...
		/**
		 * *********************************************************
		 * Check libray again and check there is nothing left unchecked
		**********************************************************
		 */
		TestProcessHelper.checkLibrary();
		...
		/**
		 * *********************************************************
		 * 3rd Merge: stat sources are NOT upated: -> "not found files" issues
		 * after merge -> Except MyTunes (Android) as linked to a device so
		 * original path and filename is known THEN change some stats in JaMuz
		 * and MyTunes (Android)
		**********************************************************
		 */
		PanelMain.selectTab(Inter.get("Label.Merge"));
		...
		**********************************************************
		...
		TestSettings.copyStatSourceDatabase("guayadeque_Device_Updated.db", "guayadeque_Device.db");
		TestSettings.copyStatSourceDatabase("mixxxdb_Device_Updated.sqlite", "mixxxdb_Device.sqlite");
		TestSettings.copyStatSourceDatabase("MyMusic60_Device_Updated.db", "MyMusic32_Device.db");
		...
		**********************************************************
		...
		TestSettings.copyStatSourceDatabase("MusicIndexDatabase_Device_Updated.db", "MusicIndexDatabase_Device.db");
